...
func (s *Storage) CreateBranch(con context.Context, branch storage.Branch) (string, error) {
	...
	var id string
	if err := stmt.Get(&id, branch); err != nil {
...
LEFT JOIN district ON district.id = district_id
LEFT JOIN station ON station.id = station_id
WHERE (branch.id = $1 OR branch.branch_name = $1 OR branch_phone_1 =$1 OR branch_phone_2=$1 OR branch_email=$1) AND  branch.deleted_at IS NULL
`

func (s *Storage) GetBranchBy(ctx context.Context, data string) (*storage.Branch, error) {
	var branch storage.Branch
	if err := s.db.Get(&branch, gbranch, data); err != nil {
